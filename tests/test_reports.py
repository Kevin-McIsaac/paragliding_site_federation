...
def test_overrides_report_resolves_keys_to_names():
    ...
    au = record("ansg", "a", name="Long Reef SE", url="https://siteguide.org.au/sites/details/9")
    entries = [{"a": "pge:1", "b": "ansg:a", "verdict": "never", "reason": "opposite aspects of one ridge"}]
...
def test_overrides_report_flags_a_key_that_no_longer_resolves():
    ...
    entries = [{"a": "pge:999", "b": "ansg:a", "verdict": "never", "reason": "gone"}]

    table = render_overrides(entries, [record("ansg", "a")], set())
...
def test_reason_containing_a_pipe_cannot_break_the_table():
    entries = [{"a": "pge:1", "b": "ansg:a", "verdict": "never", "reason": "north | south"}]
    table = render_overrides(entries, [record("pge", "1"), record("ansg", "a")], set())
...
def test_duplicates_report_shows_wind_so_deliberate_pairs_are_obvious():
    ...
    a = record("ansg", "1", name="Tasman 3", wind={"E": 1, "NE": 1})
    b = record("ansg", "2", name="Tasman 4", wind={"W": 1, "NW": 1})
...
def test_duplicates_are_split_into_a_table_per_source():
    ...
    au = (record("ansg", "a", name="Tasman 3"),
          record("ansg", "b", name="Tasman 4", lat=-33.7 - metres(35)), 35.0)

    table = render_duplicates([pge, au])

    assert "## pge — 1 pairs" in table
    assert "## ansg — 1 pairs" in table
...
def test_override_cell_is_a_complete_pasteable_entry():
    ...
    cell = override_cell(record("pge", "10714"), record("ansg", "109-238"), "always")

    assert cell == (
        '`{"a": "pge:10714", "b": "ansg:109-238", '
...
def test_override_cell_parses_as_a_valid_overrides_entry(tmp_path):
    ...
    cell = override_cell(record("pge", "1"), record("ansg", "a"), "never")
    ...
    assert loaded.never == {frozenset({"pge:1", "ansg:a"})}
...
def test_review_offers_always_and_merged_offers_never():
    ...
    pge, au = record("pge", "1"), record("ansg", "a", lat=-33.7 - metres(300))
    review = render_review([ReviewItem(pair_for(pge, au), ReviewReason.BEYOND_THRESHOLD)])
    merged = render_merged([Cluster((record("pge", "1"), record("ansg", "a")))])
...
def test_overrides_report_flags_a_forced_pair_that_did_not_apply():
    entries = [{"a": "pge:1", "b": "ansg:a", "verdict": "always", "reason": "one site"}]
    recs = [record("pge", "1"), record("ansg", "a")]

    assert "not applied" in render_overrides(entries, recs, set())
    assert "| yes |" in render_overrides(entries, recs, {frozenset({"pge:1", "ansg:a"})})
...
def test_merged_report_lists_both_source_names_and_the_distance():
    ...
    au = record("ansg", "a", name="Wagga (80m dunes)",
    ...
    assert '`{"a": "pge:1", "b": "ansg:a", "verdict": "never", "reason": ""}`' in table
...
def test_merged_report_sorts_widest_gaps_last():
    ...
    close = Cluster((record("pge", "1", name="Close PGE"),
                     record("ansg", "a", name="Close AU", lat=-33.7 - metres(30))))
    far = Cluster((record("pge", "2", name="Far PGE", lat=-34.0),
                   record("ansg", "b", name="Far AU", lat=-34.0 - metres(240))))
